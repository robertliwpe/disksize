...
then
	for i in $installist; do echo "Total File System Size for Install: $i" && ssh $i@$i.ssh.wpengine.net "cd ~/sites/$i && wget --quiet https://robertliwpe.wpengine.com/dirsizebytes.zip && mv --force dirsizebytes.zip dirsizebytes.php 2>/dev/null && wp eval-file ./dirsizebytes.php && rm -f ./dirsizebytes.php && rm -f ./dirsize* && wget --quiet https://robertliwpe.wpengine.com/dirsize.zip && mv -f dirsize.zip dirsize.php" && echo "Please access the web readout by visiting http://$i.wpengine.com/dirsize.php"; done
else
	for i in $installist; do echo "Total File System Size for Install: $i" && ssh $i@$i.ssh.wpengine.net "cd ~/sites/$i && wget --quiet https://robertliwpe.wpengine.com/dirsizebytes.zip && mv -f dirsizebytes.zip dirsizebytes.php 2>/dev/null && wp eval-file ./dirsizebytes.php && rm -f ./dirsizebytes.php"; done
fi
